...
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
...
// Добавляет новую команду в массив для создания
//
// Параметры:
//    Имя   - Строка - 
//    Метод - Строка - 
...
// Добавляет данные нового поля картинки
//
// Параметры:
//    Имя - Строка - имя элемента формы
//
...
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция ПолеКартинки(Имя) Экспорт

	ПроверитьИмя(Имя, "ПолеКартинки");

	ДобавитьЭлементФормы(Имя);
	ТипЭлемента(Тип("ПолеФормы"));
	ВидЭлемента(ВидПоляФормы.ПолеКартинки);
...
//    ВидДекорации - ВидДекорацииФормы - 
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция Декорация(Имя, ВидДекорации = Неопределено) Экспорт

	ПроверитьИмя(Имя, "Надпись");
	Если Не ВидДекорации = Неопределено И Не ТипЗнч(ВидДекорации) = Тип("ВидДекорацииФормы") Тогда
		ВызватьИсключение СтрШаблон("РеквизитСсылка %1: параметр ""ВидДекорации"" должен быть ""ВидДекорацииФормы""", Имя);
	КонецЕсли;

	ДобавитьЭлементФормы(Имя);
	ТипЭлемента(Тип("ДекорацияФормы"));
	Если Не ВидДекорации = Неопределено Тогда
		ВидЭлемента(ВидДекорации);
	КонецЕсли;
...
// Устанавливает место расположения для последующего перемещения элемента во время генерации
//
// Параметры:
...
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция Переместить(МестоРасположения = "") Экспорт
	
	Если Не ПустаяСтрока(МестоРасположения) Тогда
		ПроверитьИмя(МестоРасположения, "Переместить");
	КонецЕсли;

	Свойство("Место", МестоРасположения);	
...
Функция Обработчик(Имя, Метод) Экспорт 
	...
	Если ТекущийПоток.Вид = Неопределено Тогда
		ВызватьИсключение "Обработчик: не задан вид элемента";
	КонецЕсли;

	ДоступныеОбработчики = МассивОбработчиковЭлементовФорм(ТекущийПоток.Вид);
	Если ДоступныеОбработчики.Найти(Имя) = Неопределено Тогда
		ВызватьИсключение "Обработчик: имя обработчика не соответвует виду элемента формы";
	КонецЕсли;

	ПроверитьИмя(Метод, "Обработчик", "Метод");
...
// Устанавливает вертикальное выравнивание подчиненных элементов в группе
//
// Параметры:
//    ВертикальноеПоложение - ВертикальноеПоложениеЭлемента - вариант вертикального положения элемента
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция ВертикальноеПоложениеПодчиненных(ВертикальноеПоложение) Экспорт 

	ИмяСвойства = "ВертикальноеПоложениеПодчиненных";
	ИмяТипа = "ВертикальноеПоложениеЭлемента";

	ПроверитьТипВходящегоПараметра(ВертикальноеПоложение, ИмяСвойства, ИмяТипа);

	Свойство(ИмяСвойства, ВертикальноеПоложение);
...
// Указывает необходимость выделения отрицательных чисел красным цветом
//
// Параметры:
//    ВыделятьОтрицательные - Булево, Неопределено - значение Неопределено означает автоматическое определение
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция ВыделятьОтрицательные(ВыделятьОтрицательные) Экспорт
	
	ИмяСвойства = "ВыделятьОтрицательные";

	МассивИменТипа = Новый Массив;
	МассивИменТипа.Добавить("Булево");
	МассивИменТипа.Добавить("Неопределено");
	
	ПроверитьТипВходящегоПараметра(ВыделятьОтрицательные, ИмяСвойства, МассивИменТипа);
	ПроверитьСвойствоДоступноТолькоДляПоляВвода(ИмяСвойства);

    Свойство(ИмяСвойства, ВыделятьОтрицательные);
...
Функция Гиперссылка(Гиперссылка) Экспорт
	...
	ПроверитьСвойствоДоступноТолькоДляНадписейИКартинок(ИмяСвойства);
...
// Устанавливает горизонтальное выравнивание подчиненных элементов в группе
//
// Параметры:
//    ГоризонтальноеПоложениеПодчиненных - ГоризонтальноеПоложениеЭлемента - вариант горизонтального положения элемента
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция ГоризонтальноеПоложениеПодчиненных(ГоризонтальноеПоложениеПодчиненных) Экспорт 

	ИмяСвойства = "ГоризонтальноеПоложениеПодчиненных";
	ИмяТипа = "ГоризонтальноеоложениеЭлемента";

	ПроверитьТипВходящегоПараметра(ГоризонтальноеПоложениеПодчиненных, ИмяСвойства, ИмяТипа);

	Свойство(ИмяСвойства, ГоризонтальноеПоложениеПодчиненных);
...
// Устанавливает способ группировки дочерних элементов или способ группировки колонок
//
// Параметры:
//    Группировка - ГруппировкаПодчиненныхЭлементовФормы, ГруппировкаКолонок - 
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция Группировка(Группировка) Экспорт 
	...
	ИмяСвойства = "Группировка";
	
	МассивИменТипа = Новый Массив;
	МассивИменТипа.Добавить("ГруппировкаПодчиненныхЭлементовФормы");
	МассивИменТипа.Добавить("ГруппировкаКолонок");
	
	ПроверитьТипВходящегоПараметра(Группировка, ИмяСвойства, МассивИменТипа);
...
// Устанавливает признак наличия/отсутствия кнопки выбора в поле
//
// Параметры:
...
Функция КнопкаВыбора(КнопкаВыбора) Экспорт 
	...
	Свойство(ИмяСвойства, КнопкаВыбора);
	
	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает признак наличия/отсутствия кнопки очистки в поле
//
// Параметры:
//    КнопкаОчистки - Булево, Неопределено - 
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция КнопкаОчистки(КнопкаОчистки) Экспорт 

	ИмяСвойства = "КнопкаОчистки";

	МассивИменТипа = Новый Массив;
	МассивИменТипа.Добавить("Булево");
	МассивИменТипа.Добавить("Неопределено");

	ПроверитьТипВходящегоПараметра(КнопкаОчистки, ИмяСвойства, МассивИменТипа);
	ПроверитьСвойствоДоступноТолькоДляПоляВвода(ИмяСвойства);

	Свойство(ИмяСвойства, КнопкаОчистки);
...
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция Объединенная(Объединенная) Экспорт 

	ИмяСвойства = "Объединенная";
	ИмяТипа = "Булево";

	ПроверитьТипВходящегоПараметра(Объединенная, ИмяСвойства, ИмяТипа);

    Свойство(ИмяСвойства, Объединенная);
...
//
// Параметры:
//    РазмерКартинки - РазмерКартинки - 
//
// Возвращаемое значение:
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция РазмерКартинки(РазмерКартинки) Экспорт
	
	ИмяСвойства = "РазмерКартинки";
	ИмяТипа = "РазмерКартинки";

	ПроверитьТипВходящегоПараметра(РазмерКартинки, ИмяСвойства, ИмяТипа);
	// TODO: Для каого доступно свойство?

	Свойство(ИмяСвойства, РазмерКартинки);
...
// Устанавливает форматную строку для надписи
//
// Параметры:
...
//    ОбработкаОбъект.рсф_ПостроительФорм - эта обработка для дальнейшего вызова функций в "текучем" стиле
//
Функция ФорматнаяСтрока(ФорматнаяСтрока) Экспорт 

	ИмяСвойства = "Формат";
	ИмяТипа = "Строка";

	ПроверитьТипВходящегоПараметра(ФорматнаяСтрока, ИмяСвойства, ИмяТипа);
    // TODO: Проверить что это надпись

    Свойство(ИмяСвойства, ФорматнаяСтрока);
...
Функция ТекущийЭлемент(Имя) Экспорт 
	...
	Если Элемент = Неопределено Тогда
		...
		Иначе
			...
			ТекущийЭлемент = ПараметрыСозданияЭлементов();
			ТекущийЭлемент.Имя = Имя;
			ТекущийЭлемент.Вид = ЭлементФормы.Вид;
...
Функция СоздатьЭлемент(Элемент)
	...
	Если Не ПустаяСтрока(ПараметрыСоздания.Место) Тогда
	...
	Если не Место = Неопределено Тогда
		Если ПустаяСтрока(Место) Тогда
			Форма.Элементы.Переместить(НовыйЭлемент, Родитель);
		Иначе
...
Процедура ПроверитьИмя(Имя, ИмяФункции, НаименованиеПараметра = "Имя")

	Если Не ТипЗнч(Имя) = Тип("Строка") Тогда
		ВызватьИсключение СтрШаблон("%1: параметр ""%2"" должен быть строкового типа",
			ИмяФункции, НаименованиеПараметра);
	КонецЕсли;
	Если ПустаяСтрока(Имя) Тогда
		ВызватьИсключение СтрШаблон("%1: параметр ""%2"" должен быть заполнен",
			ИмяФункции, НаименованиеПараметра);
	КонецЕсли;
	Если Не ПроверитьНаименованиеЭлемента(Имя) Тогда
		ВызватьИсключение СтрШаблон("%1: передан параметр ""%3"" некорректного формата - ""%2""",
			ИмяФункции, Имя, НаименованиеПараметра);
	КонецЕсли;
...
Процедура ПроверитьСвойствоДоступноТолькоДляНадписейИКартинок(Имя)

	ПроверитьСвойствоДоступноТолькоДляЭлемента(Имя);

	ЭтоНадписьИлиКартинка = (ТекущийПоток.Тип = Тип("ПолеФормы")
		И (ТекущийПоток.Вид = ВидПоляФормы.ПолеНадписи
			Или ТекущийПоток.Вид = ВидПоляФормы.ПолеКартинки))
		Или ТекущийПоток.Тип = Тип("ДекорацияФормы");

	Если Не ЭтоНадписьИлиКартинка Тогда
		ШаблонИсключения = "%1 %2: установка свойства ""%1"" доступно только для поля ввода с видами ""Поле надписи""
		|и ""Поле картинки"" или для декорации любого вида";
		ТекстИсключения = СтрШаблон(ШаблонИсключения, Имя, ИмяТекущегоЭлемента());
...
Функция МассивОбработчиковЭлементовФорм(Вид)

	Массив = Новый Массив;

	Если СтрНайти(Вид, "Поле") > 0 Тогда
		Массив.Добавить("ПриИзменении");
	КонецЕсли;
	Если Вид = ВидПоляФормы.ПолеФлажка Тогда
		Возврат Массив;
	ИначеЕсли Вид = ВидПоляФормы.ПолеВвода Тогда
		Массив.Добавить("НачалоВыбора");
		Массив.Добавить("НачалоВыбораИзСписка");
		Массив.Добавить("Очистка");
		Массив.Добавить("Регулирование");
		Массив.Добавить("Открытие");
		Массив.Добавить("Создание");
		Массив.Добавить("ОбработкаВыбора");
		Массив.Добавить("ИзменениеТекстаРедактирования");
		Массив.Добавить("АвтоПодбор");
		Массив.Добавить("ОкончаниеВвдаТекста");
		Массив.Добавить("ОбработкаФормированияКоманд");
		Массив.Добавить("ДобавлениеМножественныхЗначений");
		Массив.Добавить("УдалениеМножественныхЗначений");
		Массив.Добавить("ОткрытиеМножественныхЗначений");
		Массив.Добавить("ОбработкНавигационнойСсылкиМножественныхЗначений");
	ИначеЕсли Вид = ВидПоляФормы.ПолеНадписи
		Или Вид = ВидДекорацииФормы.ДекорацияНадпись
	Тогда
		Массив.Добавить("Нажатие");
		Массив.Добавить("ОбработкНавигационнойСсылки");
	ИначеЕсли Вид = ВидПоляФормы.ПолеКартинки
		Или Вид = ВидДекорацииФормы.ДекорацияКартинка
	 Тогда
		Массив.Добавить("Нажатие");
		Массив.Добавить("НачалоПеретаскивания");
		Массив.Добавить("ПроверкаПеретаскивания");
		Массив.Добавить("ОкончаниеПеретаскивания");
		Массив.Добавить("Перетаскивание");
	ИначеЕсли Вид = ВидГруппыФормы.Страницы Тогда
		Массив.Добавить("ПриСменеСтраницы");
	Иначе
		ТекстИсключения = СтрШаблон("МассивОбработчиковЭлементовФорм: неизвестный вид элемента формы - %1",
			Вид);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
